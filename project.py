...
def home():
    ...
    return render_template("index.html", result = x,memory=str(round(float(histoo(34474)),2)),space=str(round(float(histoo(34557)),2)),cpu=str(round(float(histoo(34473)),2)))
...
def forme():
    ...
    for k in l:
        if (int(k)+1) % 2 == 0:
            ...
            imp=imp+1
    #print(l)
    return render_template('najd2.html',y=getname("command.xml"),result=l,p=a,imp=b,memory=str(round(float(histoo(34474)),2)),space=str(round(float(histoo(34557)),2)),cpu=str(round(float(histoo(34473)),2)))
...
def api_id():
    ...
    if id==3:
        ...
        time.sleep(20)
        return render_template("indexx.html", result = x , title= "Folder Size",type="Folder",memory=str(round(float(histoo(34474)),2)),space=str(round(float(histoo(34557)),2)),cpu=str(round(float(histoo(34473)),2)),the=host_id)
    elif id==1:
        path = str(request.args['path'])
        cre(path,host_id)
        time.sleep(30)
        return render_template("action.html", result = path, title= "Creation of a folder",memory=str(round(float(histoo(34474)),2)),space=str(round(float(histoo(34557)),2)),cpu=str(round(float(histoo(34473)),2)),the=host_id)
    elif id==2:
        path = str(request.args['path'])
        deel(path,host_id)
        time.sleep(30)
        return render_template("action_del.html", result = path, title= "Deletion of a folder",memory=str(round(float(histoo(34474)),2)),space=str(round(float(histoo(34557)),2)),cpu=str(round(float(histoo(34473)),2)),the=host_id)
    elif id==5:
        path = str(request.args['path'])
        id2=nameid(path,host_id)
        x=orgpack(histo(id2))
        return render_template("action_version.html", result = x,title= "Version of a Package",memory=str(round(float(histoo(34474)),2)),space=str(round(float(histoo(34557)),2)),cpu=str(round(float(histoo(34473)),2)),the=host_id)
    elif id == 6:
        path = str(request.args['path'])
